...
const showContact = async (accessToken, id, axiosJWT, dispatch) => {
    ...
    try {
        const res = await axiosJWT.get(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/v1/chat/getcontact/${id}`, {
...
const addMessage = async (accessToken, newMessage, axiosJWT, dispatch) => {
    ...
    try {
        await axiosJWT.post(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/v1/chat/addmessage`, newMessage, {
            headers: { authorization: `Bearer ${accessToken}` },
            withCredentials: true,
        })
...
const showMessage = async (accessToken, getMessage, axiosJWT, dispatch) => {
    ...
    try {
        const res = await axiosJWT.post(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/v1/chat/getmessage`, getMessage, {
            headers: { authorization: `Bearer ${accessToken}` },
            withCredentials: true,
        })
