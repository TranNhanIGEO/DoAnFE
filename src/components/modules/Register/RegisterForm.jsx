...
import { setEmail, setConfirmPassword, setPassword, setUsername, setUsernameValidate, setPasswordValidate, setConfirmPasswordValidate, setEmailValidate } from './actions';
import { FormControl, FormLabel, FormPassword, FormText, FormValidate } from 'src/components/interfaces/Form/Form';
import Button from 'src/components/interfaces/Button/Button';

const RegisterForm = () => {
    const [state, dispatched] = useReducer(reducer, initState)
    const {username, password, confirmPassword, email, usernameValidate, passwordValidate, confirmPasswordValidate, emailValidate } = state
    const getError = useSelector(state => state.auth.register?.getError)
    const dispatch = useDispatch()
    const navigate = useNavigate()

    useEffect(() => {
        getError?.type === 'username' && dispatched(setUsernameValidate(getError?.msg))
    ...
    const handleRegister = (e) => {
        ...
        if (!username && !password && !confirmPassword && !email) return (
            ...
            dispatched(setConfirmPasswordValidate('Vui lòng nhập mật khẩu xác nhận')),
            dispatched(setEmailValidate('Vui lòng nhập email'))
        )
        if (!username) return dispatched(setUsernameValidate('Vui lòng nhập tài khoản'))
        if (!password) return dispatched(setPasswordValidate('Vui lòng nhập mật khẩu'))
        if (password !== confirmPassword) return dispatched(setConfirmPasswordValidate('Mật khẩu xác nhận không khớp'))
        ...
        <div className="register-container">
            <form className='register-form' onSubmit={(e) => handleRegister(e)}>
                ...
                <FormControl>
                    ...
                    />
                    {usernameValidate &&
                        <FormValidate>{usernameValidate}</FormValidate>
                    }
                </FormControl>
                <FormControl>
                    <FormLabel 
                    ...
                    />
                    {passwordValidate &&
                        <FormValidate>{passwordValidate}</FormValidate>
                    }
                </FormControl>
                <FormControl>
                    <FormLabel 
                ...
                <FormControl>
                    ...
                    />
                    {emailValidate &&
                        <FormValidate>{emailValidate}</FormValidate>
                    }
                </FormControl>
                <Button type='submit' className='register-submit'>Đăng ký</Button>
            </form>
            <div className='register-to-login'>
                <p>Bạn đã có tài khoản?</p>
                <Link to='/admin/login' className='register-link-login'>Đăng nhập ngay</Link>
            </div>
        </div>
    );
}
