...
const AdminTable = () => {
    ...
    useEffect(() => {
        if (listUsers && prevListUsers.current !== listUsers) {
            socket.current = io(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}`)
            socket.current.on('updated-user', (data) => {
                ...
                switch (data.dml_action) {
                    case 'INSERT':
                        currentListUser.push(data)
                        prevListUsers.current = currentListUser
                        dispatch(showUserSuccess(currentListUser))  
                        break;
                        
                    case 'UPDATE':
                        const newListUpdate = currentListUser.map((element) => element.id === data.id ? data : element)
                        prevListUsers.current = newListUpdate
                        dispatch(showUserSuccess(newListUpdate))   
                        break;
                        
                    case 'DELETE':
                        const newListDelete = currentListUser.filter((element) => element.id !== data.id)
                        prevListUsers.current = newListDelete
                        dispatch(showUserSuccess(newListDelete))  
                        break;
